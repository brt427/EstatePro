...
Dataset
The project uses the California Housing Train Dataset.
...
4. Training Loop
The housing_model(alpha, num_iterations) function runs the Gradient Descent algorithm.

It iteratively updates weights to minimize the MSE.

5. Hyperparameter Tuning & Visualization
The model is tested with different Learning Rates (alpha).

A plot of MSE vs. Iterations is generated to visually verify that the model is converging (learning) correctly.

Usage
Clone the repository or download the HousingPriceMLModel.ipynb file.
...
Run the Notebook: Execute the cells sequentially to train the model.

Testing:

The final section of the notebook loads a test set. Ensure you have a test.csv available and update the path in the final cell before running.
